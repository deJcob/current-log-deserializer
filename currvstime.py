...
if __name__ == '__main__':
    ...
    for k in range(0, len(bags)):
        ...
        min_tmp = 0.0
        max_tmp = 0.0
        for j in range(0, len(laserdf['effort_0'])):
            if eff0[j] > 0.1 or eff1[j] > 0.1:
                min_tmp = tim[j-30]
                print(min_tmp)
                break
        ...
        for j, e in reversed(list(enumerate(eff0))):
            if eff0[j] > 0.1 or eff1[j] > 0.1 or vel0[j] > 0.1 or vel1[j] > 0.1:
                max_tmp = tim[j + 10]-min_tmp
                print(max_tmp)
                break

        max_x = max(max_x, max_tmp)

        axs[k].plot(laserdf['Time']-timecomp-min_tmp, laserdf['effort_0']+0.58, label='Średni prąd, silnik 0',  color=colours[1])
        axs[k].plot(laserdf['Time']-timecomp-min_tmp, laserdf['effort_1']+0.46, label='Średni prąd, silnik 1',  color=colours[2])
        axs[k].plot(laserdf['Time']-timecomp-min_tmp, laserdf['velocity_0']/10, label='Prędkość, silnik 0',  color=colours[3])
        axs[k].plot(laserdf['Time']-timecomp-min_tmp, laserdf['velocity_1']/10, label='Prędkość, silnik 1',  color=colours[4])
